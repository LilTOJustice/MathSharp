...
namespace MathSharp
{
    ...
    public interface IVec4<TSelf, TBase, TFloat, TVFloat> : IEquatable<IVec4<TSelf, TBase, TFloat, TVFloat>>
        where TSelf : IVec4<TSelf, TBase, TFloat, TVFloat>, new()
        where TBase : INumber<TBase>
        where TFloat : IFloatingPoint<TFloat>
        where TVFloat : IVec4<TVFloat, TFloat, TFloat, TVFloat>, new()
    {
        /// <summary>
        ...
        public TBase IMag2() => X * X + Y * Y + Z * Z + W * W;

        /// <inheritdoc cref="Mag"/>
        public TFloat IMag() => ToTFloat(Math.Sqrt(Convert.ToDouble(Mag2())));

        /// <inheritdoc cref="Dot"/>
        public TBase IDot(in TSelf other) => X * other.X + Y * other.Y + Z * other.Z + W * other.W;
        ...
        public TSelf IAdd(in TSelf other)
            => new TSelf { X = X + other.X, Y = Y + other.Y, Z = Z + other.Z, W = W + other.W };

        /// <summary>
        /// Computes the difference of two vectors.
        /// </summary>
        public TSelf ISub(in TSelf other)
            => new TSelf { X = X - other.X, Y = Y - other.Y, Z = Z - other.Y, W = W - other.W };
        ...
        public TSelf IMul(TBase scalar)
            => new TSelf { X = X * scalar, Y = Y * scalar, Z = Z * scalar, W = W * scalar };

        /// <inheritdoc cref="IMul(TBase)"/>
        public TVFloat IFMul(TFloat scalar)
            => new TVFloat { X = ToTFloat(X) * scalar, Y = ToTFloat(Y) * scalar, Z = ToTFloat(Z) * scalar, W = ToTFloat(W) * scalar };
        ...
        /// </summary>
        public bool IEquals(in TSelf other) => X == other.X && Y == other.Y && Z == other.Z && W == other.W;
        ...
        public string IToString() => $"<{X}, {Y}, {Z}, {W}>";
